                        ...
                        <div class="border rounded-lg p-6 bg-white">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex gap-2 items-center">
                                    <input v-model="businessNumber" type="text" maxlength="10" placeholder="'-'없이 입력" class="flex-1 border rounded px-3 py-2" @input="businessNumber = businessNumber.replace(/\D/g, '').slice(0, 10)">
                                    <button type="button" class="border px-4 py-2 rounded text-blue-600 border-blue-400 text-sm" @click="fetchBizInfo">인증하기</button>
                                </div>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex gap-2 items-center">
                                    <input v-model="permitNumber" type="text" maxlength="11" placeholder="인허가번호 11자리 입력" class="flex-1 border rounded px-3 py-2" @input="permitNumber = permitNumber.replace(/\D/g, '').slice(0, 11)">
                                    <button type="button" class="border px-4 py-2 rounded text-blue-600 border-blue-400 text-sm" @click="fetchPermitInfo">인증하기</button>
                                </div>
                            ...
                            <div class="mb-4">
                                ...
                                <input v-model="companyName" type="text" placeholder="상호" class="w-full border rounded px-3 py-2" readonly>
                            </div>
                            <!-- 대표자 명 -->
                            <div class="mb-4">
                                <label class="block font-medium mb-1">대표자 명</label>
                                <input v-model="ceoName" type="text" placeholder="대표자 명" class="w-full border rounded px-3 py-2" readonly>
                            </div>
                            <!-- 사업장 주소 -->
                            <div class="mb-4">
                                <label class="block font-medium mb-1">사업장 주소</label>
                                <input v-model="address1" type="text" placeholder="사업장 주소" class="w-full border rounded px-3 py-2 mb-2" readonly>
                            </div>
                        </div>
                        <!-- 서류첨부 안내 및 파일 업로드 UI -->
                        <div class="mt-12 mb-8">
                            ...
                            <div class="flex flex-col gap-6">
                                <div class="flex items-center gap-4">
                                    ...
                                    <input type="file" accept="image/*" @change="e => onFileChange(e, 'biz')" class="hidden" id="bizFileInput">
                                    <label for="bizFileInput" class="border px-8 py-2 rounded cursor-pointer text-center bg-white">첨부하기</label>
                                    <div v-if="bizImageUrl" class="ml-4"><img :src="bizImageUrl" alt="사업자등록증 미리보기" class="max-h-20 border" /></div>
                                </div>
                                <div class="flex items-center gap-4">
                                    <label class="w-32 font-medium">통신판매신고증</label>
                                    <input type="file" accept="image/*" @change="e => onFileChange(e, 'online')" class="hidden" id="onlineFileInput">
                                    <label for="onlineFileInput" class="border px-8 py-2 rounded cursor-pointer text-center bg-white">첨부하기</label>
                                    <div v-if="onlineImageUrl" class="ml-4"><img :src="onlineImageUrl" alt="통신판매신고증 미리보기" class="max-h-20 border" /></div>
                                </div>
                                <div class="flex items-center gap-4">
                                    <label class="w-32 font-medium">업체사진</label>
                                    <input type="file" accept="image/*" @change="e => onFileChange(e, 'shop')" class="hidden" id="shopFileInput">
                                    <label for="shopFileInput" class="border px-8 py-2 rounded cursor-pointer text-center bg-white">첨부하기</label>
                                    <div v-if="shopImageUrl" class="ml-4"><img :src="shopImageUrl" alt="업체사진 미리보기" class="max-h-20 border" /></div>
                                </div>
                            </div>
                        </div>
                        <!-- 하단 버튼 -->
                        <div class="flex justify-center mt-8">
                            <button type="submit" class="bg-gray-400 text-white px-6 py-2 rounded" :disabled="!canSubmit">제출하기</button>
                        </div>
                    </form>
                </div>
            </div>
